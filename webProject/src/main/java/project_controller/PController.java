...
public class PController {
	private static PerformanceService service = new PerformanceService();
	...
	public static void main(String[] args) {
		...
		a: while (isRun) {
			...
			b: while (isRun) { // 로그인 상태인 경우
				System.out.println();
				System.out.println("=======================================================");
				System.out.println("1.공연 정보 | 2.관심공연 등록 | 3.마이페이지 | 4.탈퇴 | 0.로그아웃");
				System.out.println("=======================================================");
				System.out.println("선택>>");
				job = sc.nextInt();
				if (job == 0) { // 0.로그아웃
				...
				if (job == 1) {
					...
					case 1: {
						...
						List<PerformanceVO> pList = service.getAllPerformances();
						RView.print(pList);
						break;
					}
					case 2: {
						System.out.println("공연시작일 순으로 조회");
						List<PerformanceVO> pListByStartDate = service.getPerformancesByStartDate();
					...
					case 3: {
						...
						case 1: {
							...
							List<PerformanceVO> pListByName = service.searchPerformancesByName(searchName);
						...
						case 2: {
							...
							List<PerformanceVO> pListByDirector = service.searchPerformancesByDirector(searchDirector);
					...
					/*
					 * } else if (job == 2) { // 2. 관심 공연 등록
					 */

				} else if (job == 3) {
					System.out.println("마이페이지");
					System.out.println("============================================");
					System.out.println("1.내 정보 조회 | 2.변경 | 3.관심공연 확인 | 0.돌아가기");
					System.out.println("============================================");
					System.out.print("선택>> ");
					job = sc.nextInt();
					switch (job) {
					...
					case 2: {
						...
						System.out.print("변경할 할아이디를 입력하세요: ");
						String reuser_id = sc.next();
						System.out.print("변경할 이름을 입력하세요: ");
						String reuser_name = sc.next();
						System.out.print("변경할 비밀번호를 입력하세요: ");
						String reuser_password = sc.next();
						System.out.print("변경할 전화번호를 입력하세요: ");
						String reuser_phone = sc.next();
						System.out.print("변경할 이메일을 입력하세요: ");
						String reuser_email = sc.next();
                         		
						UserTableVO user = uservice.getUpdateUser(user_id, reuser_id, reuser_name,reuser_password, reuser_phone, reuser_email);
						RView.print(user);
						break;
